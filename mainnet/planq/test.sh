...
PLANQ_DENOM=aplanq
PLANQ_EXPONENT=18
PLANQ_URL=https://snap.kynraze.com/planq/snapshot-latest.tar.lz4

if [ "$1" == "install_node" ]; then
    # Function to install the node
    install_node() {
        # Isi dari fungsi install_node di sini
        echo "Running the install_node function..."
        # Set Vars
        read -p "[ENTER YOUR NODE] > " PLANQ_NODENAME
        read -p "[ENTER YOUR PORT] > (Recommendation: Two digits or more) " PLANQ_PORT
echo ""
...
echo -e "EXPONENT       : \033[38;2;4;204;255m$PLANQ_EXPONENT\e[0m"
echo ""
...
sudo systemctl start $PLANQ
    ...
    # Panggil fungsi install_node
    install_node
elif [ "$1" == "install_snapshot" ]; then
    # Function to install the snapshot
    install_snapshot() {
        # Update & install lz4(Required)
sudo apt install snapd lz4 -y

# Reset Chain Data
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/$PLANQ_FOLDER/config/config.toml
sudo systemctl stop $PLANQ
cp $HOME/$PLANQ_FOLDER/data/priv_validator_state.json $HOME/$PLANQ_FOLDER/priv_validator_state.json.backup
rm -rf $HOME/$PLANQ_FOLDER/data

# Download Data Snapshot
curl -o - -L $PLANQ_URL | lz4 -c -d - | tar -x -C $HOME/$PLANQ_FOLDER
mv $HOME/$PLANQ_FOLDER/priv_validator_state.json.backup $HOME/$PLANQ_FOLDER/data/priv_validator_state.json

# Restart
sudo systemctl restart $PLANQ
sudo journalctl -fu $PLANQ -o cat
        echo "Running the install_snapshot function..."
    }

    # Panggil fungsi install_snapshot
    install_snapshot
else
