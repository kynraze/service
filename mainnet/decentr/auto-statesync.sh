...
echo ""
echo -e "YOUR NODE NAME : \e[1m\e[31m$DECENTR_NODENAME\e[0m"
echo -e "NODE CHAIN ID  : \e[1m\e[31m$DECENTR_ID\e[0m"
echo -e "NODE PORT      : \e[1m\e[31m$DECENTR_PORT\e[0m"
echo ""
...
# Set peers and seeds
PEERS="$(curl -sS https://https://decentr-rpc.kynraze.com/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
SEEDS=""
...
STATE_SYNC_RPC="https://decentr-rpc.kynraze.com:443"
LATEST_HEIGHT=$(curl -s $STATE_SYNC_RPC/block | jq -r .result.block.header.height)
SYNC_BLOCK_HEIGHT=$(($LATEST_HEIGHT - 1000))
SYNC_BLOCK_HASH=$(curl -s "$STATE_SYNC_RPC/block?height=$SYNC_BLOCK_HEIGHT" | jq -r .result.block_id.hash)

sed -i.bak -e "s|^enable *=.*|enable = true|" $HOME/$DECENTR_FOLDER/config/config.toml
sed -i.bak -e "s|^rpc_servers *=.*|rpc_servers = \"$STATE_SYNC_RPC,$STATE_SYNC_RPC\"|" \
    $HOME/$DECENTR_FOLDER/config/config.toml
